...
class PostgresAPI extends DataSource {
  ...
  async posts() {
    const res = await this.store.Post.findAll();
    return res;
  }


  async createPost(description, imageUrl) {
    let id = uuidv4();

    const post = await this.store.Post.create({
      id: id,
  ...
  async deletePost(id) {
    ...
    return await !!this.store.Post.destroy({
      where: {
        id: id
      }
    }).then(() => {
